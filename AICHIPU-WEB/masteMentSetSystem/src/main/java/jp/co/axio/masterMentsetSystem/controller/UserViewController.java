...
public class UserViewController extends accessController {
	...
	@ResponseBody
	@Transactional
    public AjaxResultDto userDetailOp(@RequestBody UserRegistForm form, RedirectAttributes redirectAttributes) throws Exception {
    	...
    	if(UserRegistForm.OPDIV_DEL.equals(form.getViewPageOpDiv())) {
    		...
    		//操作ログを登録
    		commonService.insOpelog(MmsConstants.FUNC_ID_1000, MmsConstants.FUNC_ID_1000_LABEL, form.getRiYouShaId(), "ユーザをロックしました。");
    	}else if(UserRegistForm.OPDIV_UNLOCK.equals(form.getViewPageOpDiv())) {
    		userViewService.userUnLock(form.getRiYouShaId());
			//操作ログを登録
			commonService.insOpelog(MmsConstants.FUNC_ID_1000, MmsConstants.FUNC_ID_1000_LABEL, form.getRiYouShaId(), "ユーザをロック解除しました。");
    	}
