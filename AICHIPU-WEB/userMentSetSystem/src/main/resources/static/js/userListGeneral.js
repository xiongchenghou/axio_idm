...
namespace('axio.userListGeneral');
...
$(function() {
	...
	if ($("#popupShowFlg").val() == "ON") {
		...
		userListGeneralConfirm(null, m, function(){
			//検索処理
			$.ajax({
	    		url: "./userListGeneralSearch",
	    		type: "POST",
	...
	function searchBySort(e){
		...
		//検索処理
		$.ajax({
    		url: "./userListGeneralChangeOrder",
    		type: "POST",
    		data: $("#userListForm").serialize(),
    		success: function(result){
    			$('body').html(result);
	...
	$('#prePageBtn').click(function(e) {
		...
		//検索処理
		$.ajax({
    		url: "./userListGeneralShowPrePage",
    		type: "POST",
    		data: $("#userListForm").serialize(),
    		success: function(result){
    			$('body').html(result);
	...
	$('#nextPageBtn').click(function(e) {
		...
		//検索処理
		$.ajax({
    		url: "./userListGeneralShowNextPage",
    		type: "POST",
    		data: $("#userListForm").serialize(),
    		success: function(result){
    			$('body').html(result);
     ...
     * ボタン押下時確認ダイヤログメッセージ
     */
	function userListGeneralConfirm(e,message,okcallback,ngcallback) {
		var that = this;
		if (message) {
			confirmText = message;
		}
