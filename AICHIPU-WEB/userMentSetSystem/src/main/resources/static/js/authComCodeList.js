...
namespace("axio.authComCodeList");
...
//		alert("checkAll change");
		...
		if (isNaN(cnt)) cnt = 0;
		for (var i = 0; i < cnt; i++) {
			$("#companyCodeCheck_" + i).prop("checked", checked);
		}
	});
...
//		alert("Search click");
		...
		if (isNaN(cnt)) cnt = 0;
		for (var i = 0; i < cnt; i++) {
			if ($("#companyCodeCheck_" + i).prop("checked")) {
				vals.push({"companyCode":"" + $("#companyCodeCheck_" + i).val()});
			}
		}
...
//		alert("Ok click");
		...
		for (var i = 0; i < cnt; i++) {
			if ($("#companyCodeCheck_" + i).prop("checked")) {
				vals.push({"company":"" + $("#companyCodeCheck_" + i).val(), "name":"" + $("#companyNameHidden_" + i).val()});
			}
		}
