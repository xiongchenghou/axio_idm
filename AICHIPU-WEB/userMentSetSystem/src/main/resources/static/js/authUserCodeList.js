...
namespace("axio.authUserCodeList");
...
//		alert("checkAll change");
		...
		if (isNaN(cnt)) cnt = 0;
		for (var i = 0; i < cnt; i++) {
			$("#userCodeCheck_" + i).prop("checked", checked);
		}
	});
...
//		alert("Search click");
		...
		if (isNaN(cnt)) cnt = 0;
		for (var i = 0; i < cnt; i++) {
			if ($("#userCodeCheck_" + i).prop("checked")) {
				var code =  $("#userCodeCheck_" + i).val().split(":");
				vals.push({"userCode":"" + code[0]});
			}
		}
...
//		alert("Ok click");
		...
		for (var i = 0; i < cnt; i++) {
			if ($("#userCodeCheck_" + i).prop("checked")) {
				var code =  $("#userCodeCheck_" + i).val().split(":");
				vals.push({"user":"" + code[0],"name":"" + $("#userNameHidden_" + i).val()});
			}
		}
		vals = vals.sort(function(val1, val2) {
			return (val1.user > val2.user ? 1 : -1);
		});
