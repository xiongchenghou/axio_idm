...
$(function () {
	...
	if ($("#grpFlag2").prop("checked")) {
		$("#manualGrpName").attr('readonly', false);
	}

	if ($("#mailGrpFlag2").prop("checked")) {
		$("#ManualmailGrpName").attr('readonly', false);
	}
			...
			buttons: [
				{
					...
					click: function () {
						...
						$('#wanCheck').val("1");
						$('#wanMessage').val("");
						$('#SecurityGrpRegForm').attr('action', url);
	 ...
	 * ダイヤログコールバック
	 */
	// axio.securityGrReg.dailogCallback = function dailogCallback(res, err) {
	 ...
	 $("#backbtn").on('click', function (e) {
		...
		if ($("#changedFlg").val() == '1' || $("#editFlg").val() == '1') {
			showConfirm(e, message,
				...
				function () {
					$('#SecurityGrpRegForm').attr('action', './login');
					$('#SecurityGrpRegForm').submit();
				});
		} else {
			// グループ検索モーダル画面呼び出す
	...
	$('#setSaveBtn').on('click', function (e) {
		...
		// if ($("#organizationCodeName").val() == "") {
		// 	var title = "確認";
		// 	var message = "<p>組織を選択してください。</p>";
		...
		showConfirm(e, message,
			function () {
				//検索処理
				$.ajax({
					url: "./securityGrpRegSave",
					type: "POST",
					data: $("#SecurityGrpRegForm").serialize(),
					success: function (result) {
						$('body').html(result);
						//ボタン押下開放
						isPosting = false;
	...
	$('#invalid').on('click', function (e) {
		...
		var message = "<p>セキュリティグループ情報を無効化します。<br/> よろしいですか？</p>";
		...
		showConfirm(e, message,
			function () {
				...
				// 無効化
				$('#SecurityGrpRegForm').attr('action', './securityGrpRegInvalid');
				$('#SecurityGrpRegForm').submit();
	...
	$('#clear').on('click', function (e) {
		...
		var message = "<p>セキュリティグループ情報をクリアします。<br/> よろしいですか？</p>";
		...
		showConfirm(e, message,
			function () {
				...
				// 初期画面
				$('#SecurityGrpRegForm').attr('action', './securityGrpReg');
				$('#SecurityGrpRegForm').submit();
	...
	$('.select_btn').on('click', function (e) {
		...
		// セキュリティグループ選択
		$('#SecurityGrpRegForm').attr('action', './securityGrpSetSelect');
		$('#SecurityGrpRegForm').submit();
	});
 ...
 */
function search(e) {
	$('#SecurityGrpRegForm').attr('action', './securityGrpSetRedraw');
	$('#SecurityGrpRegForm').submit();
};
